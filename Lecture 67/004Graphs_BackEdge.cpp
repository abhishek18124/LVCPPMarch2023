...
public :
	...
	bool dfsHelper(T source,
		...
		for (T neighbour : neighbourList) {
			if (visited.find(neighbour) == visited.end()) {
				...
				// check if neighbour ------> source path exists

				// check if neighbour is on the function call stack

				if (stackState.find(neighbour) != stackState.end()) {

					// neighbour is present on the function stack

					// i.e. there exists a directed path from neighbour to source

					// i.e. source -> neighbour is a backedge
	...
	bool isBackEdgePresent() {
		bool flag = false;           // assume back-edge / cycle is not present
