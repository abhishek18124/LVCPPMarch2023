...
TreeNode* buildTree(int* pre, int* in, int s, int e) {
	...
	// use the inOrder traversal of the leftSubtree of 'x' in[s...k-1] to build the leftSubtree of 'x'
	root->left = buildTree(pre, in, s, k - 1);

	// use the inOrder traversal of the rigthSubtree of 'x' in[k+1...e] to build the rightSubtree of 'x'
