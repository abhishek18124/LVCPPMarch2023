...
bool edgeComparator(edge<T> e1, edge<T> e2) {
	// return e1.w < e2.w;
	if (e1.w < e2.w) {
		// no swap needed to sort the vector in the increasing order
		return true;
	}
	// swap needed to sort the vector in the increasing order
	return false;
...
public :
	...
	// assume graph has |V| vertices and |E| edges

	// time : |V|.c (for creating sets) + |E|log|E| (for sorting edgeList) + |E| (to choose |V|-1 safe edges)
	// time : |V| + |E|log|E| + |E| ~ O(|E|log|E|)
