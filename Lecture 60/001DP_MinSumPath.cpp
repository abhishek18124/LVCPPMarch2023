...
int minSumPath(vector<vector<int>> grid, int i, int j) {
	...
	int X = minSumPath(grid, i - 1, j);
	int Y = minSumPath(grid, i, j - 1);
...
int minSumPathTopDown(vector<vector<int>> grid, int i, int j, vector<vector<int>>& dp) {
	...
	int X = minSumPathTopDown(grid, i - 1, j, dp);
	int Y = minSumPathTopDown(grid, i, j - 1, dp);
...
int minSumPathBottomUp(vector<vector<int>>& grid, int m, int n) {
	...
	return dp[m - 1][n - 1]; // at the (m-1, n-1)th index of dp[][], we store f(m-1, n-1)
